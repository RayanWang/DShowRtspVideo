...
Boolean MediaSubsession::initiate(Boolean rtcpInstant, int useSpecialRTPoffset) {
  ...
  do {
	...
	fReadSource = fRTPSource
	  = MPEG4ESVideoRTPSource::createNew(env(), fRTPSocket,
					     fRTPPayloadFormat,
					     fRTPTimestampFrequency);
      //} else if (strcmp(fCodecName, "MPEG4-GENERIC") == 0) { modify by jiok for X-RN-MP4
	...
	fReadSource = fRTPSource
	  = MPEG4GenericRTPSource::createNew(env(), fRTPSocket,
					     ...
					     fIndexdeltalength);
	  } else if (strncmp(fCodecName, "X-RN-MP4", strlen("X-RN-MP4")) == 0) {

		  strncpy(fCodecName, "MPEG4-GENERIC", strlen("MPEG4-GENERIC"));
		  fReadSource = fRTPSource
			  = MPEG4GenericRTPSource::createNew(env(), fRTPSocket,
			  fRTPPayloadFormat,
			  fRTPTimestampFrequency,
	...
	// add by jiok 1.9.2009
	if (rtcpInstant)
	{
		if (fRTPSource != NULL) {
		  unsigned totSessionBandwidth = 25;//500; // HACK - later get from SDP#####
