...
char* RTSPClient::describeURL(char const* url, Authenticator* authenticator,
  ...
  do {
   ...
   int Strlen;
	...
	if (strncmp(serverType, "CoRaise", 7) == 0) fServerIsCoship = True;
	if (strncmp(serverType, "MDN_HWPSS", 9) == 0) fServerIsMDN_HWPSS = True;
...
#ifdef SESSIONID	
	}else if (_strncasecmp(lineStart, "Session: ", 9) == 0){
...
*/
      ...
      if (connect(fInputSocketNum, (struct sockaddr*) &remoteName, sizeof remoteName) != 0) {
        ...
        //if (errno != WSAEINPROGRESS) { modify by jiok 
