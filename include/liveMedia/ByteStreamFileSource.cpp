...
ByteStreamFileSource::~ByteStreamFileSource() {
  ...
  envir().taskScheduler().turnOffBackgroundReadHandling((int)fileno(fFid)); //add int by jiok for type match
#endif
...
void ByteStreamFileSource::doGetNextFrame() {
  ...
  if (!fHaveStartedReading) {
    // Await readable data from the file: add int by jiok for match type
    envir().taskScheduler().turnOnBackgroundReadHandling((int)fileno(fFid),
...
#ifndef READ_FROM_FILES_SYNCHRONOUSLY//add int by jiok for match type
  envir().taskScheduler().turnOffBackgroundReadHandling((int)fileno(fFid));
